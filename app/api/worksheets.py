...
def write_to_worksheet(
    ...
    start_cell: str = Query("A1", description="The cell where teh data insertions should start"),
    current_user= Depends(get_current_user),
    service: GoogleSheetsService = Depends(get_google_sheets_service),
):
...
def append_records_to_worksheet(
    spreadsheet_id: str = Path(..., description="The ID of the Google Spreadsheet"),
    worksheet_name: str = Path(..., description="The name of the worksheet"),
    records: List[Dict[str, Any]] = Body(..., description="The records to append to the worksheet"),
    auto_increment_id: bool = Query(False, description="Whether to add an auto-incrementing the 'id' column"),
    current_user= Depends(get_current_user),
    service: GoogleSheetsService = Depends(get_google_sheets_service),
):
